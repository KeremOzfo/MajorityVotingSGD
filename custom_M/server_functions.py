...
def custom_m(w_grad,momentum_vals, momentum_constant,sparsity):
    sparse_k = math.ceil(w_grad.numel() / sparsity)
    vals, inds = torch.topk(w_grad.abs(), k=sparse_k,dim=0)
    momentum_vals.mul_(momentum_constant)
    momentum_vals[inds] += 1


def modify_mask(w_grad,mask, errors,sparsity,device):
    sparse_k = math.ceil(w_grad.numel() / sparsity)
    error_grads = w_grad.mul(1-mask)
    inds = torch.topk(error_grads.abs(),k=sparse_k, dim=0)[1]
    new_m = torch.clone(mask).to(device)
    new_m[inds] = 1
    errors[inds] = 0
    return new_m

def modify_mask2(w_grad,mask, errors,sparsity,device):
    sparse_k = math.ceil(w_grad.numel() / sparsity)
    l_errors = torch.clone(errors).to(device)
    l_errors.mul_(1-mask)
    inds = torch.topk(l_errors.abs(),k=sparsity,dim=0)[1]
    l_mask = torch.zeros_like(w_grad)
    l_mask[inds] = 1
    sparsed_grad = ((errors.mul(l_mask)) + (w_grad.add(errors.mul(1-l_mask)))).mul(mask)
    new_errors = ((errors.mul(1-l_mask)).add(w_grad)).mul(1-mask)

    return sparsed_grad, new_errors
