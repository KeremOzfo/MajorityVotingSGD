...
def worker_voting2(prev_local_mask, worker_model, args, device):
    sparse_val = math.ceil(worker_model.numel() / args.majority_Sparsity)
    k_sparse = math.ceil(worker_model.numel() / args.K_val)
    w_val, w_inds = torch.topk(worker_model.abs(), k = sparse_val, dim=0)
    current_mask = torch.zeros_like(prev_local_mask).to(device)
    current_mask[w_inds] =1
    worker_drop =prev_local_mask.mul(1-current_mask)
    worker_add = current_mask.mul(1-prev_local_mask)
    drop_Val = torch.sum(worker_drop).int()
    k_sparse = np.min([drop_Val,k_sparse]).__int__()

    m_dropTarget = worker_model.mul(worker_drop)
    m_addTarget = worker_model.mul(worker_add)
    drop_inds = torch.topk(m_dropTarget.abs(),k=drop_Val,dim=0)[1]
    drop_inds = torch.flip(drop_inds, dims=[0])
    add_inds = torch.topk(m_addTarget.abs(), k=k_sparse, dim=0)[1]
    prev_local_mask[add_inds] = 1
    prev_local_mask[drop_inds[0:k_sparse]] = 0
    return prev_local_mask
...
def accumulate_Errors(args, epoch,run,trun,net_users,errors,device):
    num_client = args.num_client
    difmodels = []
    for cl in range(num_client):
        model_flat = get_model_flattened(net_users[cl], device)
...
def quantize(grad_flat,args,device):
    sparseCount = torch.sum(grad_flat!=0)
    sparseCount= sparseCount.__int__()
    vals, ind = torch.topk(grad_flat.abs(),k=sparseCount, dim=0)
    custom_denom = (vals[0] / vals[sparseCount-1]) ** (1/args.num_groups)
    top_Val = vals[0]
    sign_mask = torch.sign(grad_flat)
    clone_grad = torch.zeros_like(grad_flat).to(device)
